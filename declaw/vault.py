...
# Hidden tenancy defaults — teams, members, and environments are not
# exposed in the client surface. Every secret is stored under a single
# auto-provisioned "default" team and "prod" environment, and referenced
# by bare name. The backend keeps the full tenancy model; this is purely
# a client-side simplification.
...
class VaultClient:
    """Sync client for Declaw vault management.
    ...
    Usage:
        client = VaultClient()  # uses DECLAW_API_KEY env var
        secret = client.create_secret("sk-...", provider="openai", name="openai")
    """
    ...
    def __init__(
        ...
        self._config = config  # used by expand_vault_refs to build VaultClient
        # Per-instance cache so team resolution happens at most once.
    ...
    def _resolve_default_team(self, create: bool = True) -> Optional[str]:
        ...
        on the instance so resolution happens at most once per VaultClient.
        """
        ...
        resp = self._client.post("/teams", json={"name": _DEFAULT_TEAM_NAME})
        self._team_id_cache = resp.json()["team_id"]
        return self._team_id_cache

    def _ensure_default_env(self, team_id: str) -> None:
        ...
        """
        resp = self._client.get(f"/teams/{team_id}/environments")
        ...
        try:
            self._client.post(f"/teams/{team_id}/environments", json={"name": _DEFAULT_ENV_NAME})
        except Exception:
            # A racing creator may have made it first; re-check.
            resp2 = self._client.get(f"/teams/{team_id}/environments")
            for e in resp2.json().get("environments", []):
                if e.get("name") == _DEFAULT_ENV_NAME:
                    return
            raise

    def _resolve_secret_id(self, team_id: str, name: str) -> str:
        """Map a secret name to its secret_id within the default team."""
        resp = self._client.get(f"/teams/{team_id}/vault/secrets")
    ...
    def create_secret(
        ...
        *,
        ...
        """
        team_id = self._resolve_default_team(create=True)
        assert team_id is not None  # always non-None when create=True
        self._ensure_default_env(team_id)
    ...
    def delete_secret(self, name: str) -> None:
        ...
        if team_id is None:
            raise ValueError(f"vault secret {name!r} not found")
        secret_id = self._resolve_secret_id(team_id, name)
        self._client.delete(f"/teams/{team_id}/vault/secrets/{secret_id}")

    def update_scopes(self, name: str, scopes: List[Union[VaultScope, Dict[str, Any]]]) -> None:
        """Replace a secret's injection scopes (by name); the value is unchanged.

        Use this to change a secret's destination(s) or injection format in
    ...
    def list_presets(self) -> List[VaultPreset]:
        """Return the built-in provider catalog (templates only, no secret material)."""
        resp = self._client.get("/vault/presets")
        data = resp.json()
...
# ---------------------------------------------------------------------------
# Shared vault_refs expander (used by both sync and async sandbox create)
# ---------------------------------------------------------------------------


def expand_vault_refs(client: VaultClient, refs: Dict[str, str]) -> Dict[str, str]:
    """Rewrite bare secret names in *refs* to full vault:// URIs.

    Values already in ``vault://`` form are passed through unchanged.
    Bare names are expanded to ``vault://<team_id>/prod/<name>``.

    Args:
        client: A :class:`VaultClient` instance (used for team resolution).
